...
def upload(request):
    ...
    if os.path.exists(path):
        ...
        thumbnail_path = thumbnail_root + '_' + \
    ...
    # サムネイル実装
    if thumbnail_path:
        file_model.thumbnail_path = os.path.join(
            '/static/files/thumbnail/'+os.path.basename(thumbnail_path))
    else:
        # サムネイルがないときはNOTHUMBNAIL
        file_model.thumbnail_path = 'https://user-images.githubusercontent.com/48968940/74902177-970dbf00-53e8-11ea-94d5-a93d440ba732.jpg'
...
def list(request):
    ...
    extension = File.objects.values(
        'extension').order_by('extension').distinct()
    projects_name = File.objects.values(
        'projects_name').order_by('projects_name').distinct()
